...
public class GeneralRepository {
    ...
    public <T> T get(Class<T> tClass, Object id) {
        ...
        try {
            ...
            List<String> primaryKeys = getPrimaryKeys(TableMetadata.getTableName(tClass), dbConnection);
            ...
            StringBuilder sqlBuilder = new StringBuilder("select * from ")
                    .append(TableMetadata.getTableName(tClass))
    ...
    private boolean isInvalidForInsertOrUpdate(Object t)throws Exception{
        ...
        return tableMetadata != null &&
                !tableMetadata.isAutoIncrement() &&
                ReflectUtility.isPrimaryKeyEmpty(t, tableMetadata.getSinglePrimaryKey());
    }
    ...
    private void processRelationalInsertOrUpdate(Object t
        ...
        for(RelationInfo annotation : tableMetadata.getRelationInfos(OneToMany.class)){
            ...
            if(childs != null){
                updateChildsWithAutoGeneratedKeys(childs, generatedKey, annotation.getMatchingColumn());
                for(Object o : childs){
    ...
    private void updateObjectWithAutoGeneratedKey(Object o, Object autoGeneratedKey) throws Exception{
        ...
        TableMetadata tableMetadata =  ConfigManager
                                            .getInstance()
                                                .getTableMetadata(o.getClass());

        ReflectUtility.mapValue(o
                , tableMetadata.getSinglePrimaryKey()
                , tableMetadata.getClassKeyType(tableMetadata.getSinglePrimaryKey())
                , autoGeneratedKey);
    }
    ...
    public <T> List<T> getAll(Class<T> tClass) {
        return getAll(tClass, "select * from " + TableMetadata.getTableName(tClass));
    }

    public <T> List<T> getAll(Class<T> tClass, int limit) {
        return getAll(tClass, "select * from " + TableMetadata.getTableName(tClass) + " limit " + limit);    }

    public <T> List<T> getAll(Class<T> tClass, int limit, int offset) {
        return getAll(tClass, "select * from " + TableMetadata.getTableName(tClass) + " limit " + limit + " offset " + offset);
    }
    ...
    private  List<String> findPrimaryKeys(Object row, Connection connection) {
        ...
        if(primaryKeys.isEmpty()){
            primaryKeys = getPrimaryKeys(TableMetadata.getTableName(row.getClass()), connection);
        }
        return primaryKeys;
